...
import { Calculator, CheckSquare, ArrowLeft, ArrowRight } from "lucide-react";
...
interface FormData {
  ...
  hosting: "bim-africa" | "client" | "";
  domain: "bim-africa" | "client" | "";
...
const QuotationTool: React.FC = () => {
  ...
  const calculatePricing = () => {
    ...
    const pagesPrice = formData.websiteType === "landing" ? 0 : pagesPriceMap[formData.pages] || 0;
  ...
  const validateStep = (step: number): boolean => {
    ...
    if (step === 2) {
      ...
      if (formData.websiteType !== "landing" && !formData.pages) newErrors.pages = "Select number of pages";
    ...
    if (Object.keys(newErrors).length > 0) {
      window.alert(newErrors.whatsappNumber || newErrors.email || newErrors.fullName || newErrors.country || newErrors.products || newErrors.insertProducts || newErrors.pages || newErrors.designStyle || newErrors.timeline || newErrors.hosting || newErrors.domain);
      return false;
    }
    return true;
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const message = ` BIM Africa - Your Website Quotation\n\nHi ${formData.fullName},\n\nThank you for using our instant quotation tool.\nHere is your quote summary:\n\nCountry: ${formData.country || "(not provided)"}\nWebsite Type: ${formData.websiteType}\nPages: ${formData.pages}\nDesign: ${formData.designStyle}\nFeatures: ${formData.features.join(", ") || "None"}\nTimeline: ${formData.timeline}\nHosting: ${formData.hosting}\nDomain: ${formData.domain}\nWhatsApp Number: ${formData.whatsappNumber}\n\nFinal Price: ${finalCurrency} ${Math.round(finalAmount).toLocaleString()}\n\nWe will contact you shortly to discuss further.\n\nBest Regards,\nBIM Africa Team\nwww.bim.africa\n`;

      const fd = new FormData();
      fd.append("name", formData.fullName);
      fd.append("email", formData.email);
      fd.append("phone", formData.whatsappNumber);
      fd.append("message", message);
      fd.append("_cc", formData.email); // send copy to the client's email
      fd.append("_captcha", "false");
      fd.append("_template", "table");

      await fetch("https://formsubmit.co/sales@bim.africa", {
        method: "POST",
        body: fd,
      });
      ...
      window.alert("Failed to send quote. Please try again or check the console.");
    } finally {
          ...
          <div className="bg-white rounded-2xl shadow-xl p-8 md:p-12">
            ...
            <div className="bg-gray-50 rounded-lg p-6 mb-8">
              ...
              <div className="space-y-2 text-sm">
                ...
                <div className="flex justify-between">
                  <span>Features:</span>
                  <span className="font-medium capitalize">{formData.features.join(", ") || "None"}</span>
                </div>
                <div className="flex justify-between">
                  <span>Timeline:</span>
                ...
                <div className="flex justify-between">
                  <span>Number:</span>
                  <span className="font-medium capitalize">{formData.whatsappNumber}</span>
                </div>
                <hr className="my-4" />
                <div className="flex justify-between text-lg font-bold">
                  <span>Final Price:</span>
                  <span className="text-blue-600">{finalCurrency} {Math.round(finalAmount).toLocaleString()}</span>
                </div>
              </div>
            </div>
      ...
      <div className="max-w-4xl mx-auto">
        <div className="text-center mb-12">
          ...
          <h1 className="text-4xl font-bold text-gray-900 mb-4">
            BIM Africa - Instant Website Quotation Tool
          </h1>
          ...
          <div className="flex items-center justify-center space-x-4">
            {[1, 2, 3].map((s) => (
              <div key={s} className="flex items-center">
                ...
                {s < 3 && (
                  <div className={`w-16 h-1 mx-2 ${currentStep > s ? "bg-[#ff6f61]" : "bg-gray-200"}`} />
                )}
              </div>
            ))}
          </div>
        ...
        <div className="bg-white rounded-2xl shadow-xl p-8 md:p-12">
          ...
          {/* Step 2: Website Details (original Step 1 was moved here) */}
          {currentStep === 2 && (
            <div className="space-y-8">
          ...
          {currentStep === 3 && (
            <div className="space-y-8">
              ...
              <div>
                ...
                <div className="space-y-3">
                  {[
                    { value: "bim africa", label: "BIM Africa purchases (non-premium) " },
    ...
    <div className="space-y-2 text-sm text-blue-800">
      ...
      {(() => {
        const convertPrice = (price: number) => {
          ...
          return countryRule.op === "divide"
            ? base / countryRule.rate
            : base / (1 / countryRule.rate);
        };
